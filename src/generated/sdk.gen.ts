...
/**
 * Get root account note
 * Retrieve the note for the root account (project's budget note).
 *
 * Notes are stored as rich text and can be returned in multiple formats:
 * - `html`: Rendered HTML from TipTap editor
 * - `json`: TipTap JSON document structure
 * - `text`: Plain text representation
 * - `all`: All three formats (default)
 *
 */
export const getRootAccountNote = <ThrowOnError extends boolean = false>(options: Options<GetRootAccountNoteData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetRootAccountNoteResponses, GetRootAccountNoteErrors, ThrowOnError>({
        security: [
            {
                name: 'X-API-Key',
                type: 'apiKey'
...
/**
 * Get budget line note
 * Retrieve the note for a specific budget line.
 *
 * Notes are only available for account-type lines. Subtotals, markups, and fringe lines
 * will return a null note.
 *
 * The lineId parameter uses the same resolution logic as the getBudgetLine endpoint:
 * - With `idMode=user` (default): Account codes, subtotal aliases, or partial descriptions
 * - With `idMode=system`: System-generated IDs (UUIDs/nanoids)
 *
 */
export const getLineNote = <ThrowOnError extends boolean = false>(options: Options<GetLineNoteData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetLineNoteResponses, GetLineNoteErrors, ThrowOnError>({
        security: [
            {
                name: 'X-API-Key',
                type: 'apiKey'
...
/**
 * Batch create actuals
 * Create multiple actual entries in a single request
 */
export const batchCreateActuals = <ThrowOnError extends boolean = false>(options: Options<BatchCreateActualsData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<BatchCreateActualsResponses, BatchCreateActualsErrors, ThrowOnError>({
        security: [
            {
                name: 'X-API-Key',
                type: 'apiKey'
...
/**
 * Parse transaction attachments
 * Extract structured document details (totals, merchants, dates, line items, etc.) from attachments linked to a transaction. Useful for receipts, invoices, statements, and other supporting documents.
 *
 */
export const parseTransactionAttachments = <ThrowOnError extends boolean = false>(options: Options<ParseTransactionAttachmentsData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<ParseTransactionAttachmentsResponses, ParseTransactionAttachmentsErrors, ThrowOnError>({
        security: [
            {
                name: 'X-API-Key',
                type: 'apiKey'
...
/**
 * Parse actual attachments
 * Extract structured document details (totals, vendors, dates, line items, etc.) from attachments on an actual. Works with invoices, receipts, statements, timesheets, purchase orders, contracts, and miscellaneous uploads.
 *
 */
export const parseActualAttachments = <ThrowOnError extends boolean = false>(options: Options<ParseActualAttachmentsData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<ParseActualAttachmentsResponses, ParseActualAttachmentsErrors, ThrowOnError>({
        security: [
            {
                name: 'X-API-Key',
                type: 'apiKey'
